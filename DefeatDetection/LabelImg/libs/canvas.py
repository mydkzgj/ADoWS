...
class Canvas(QWidget):
    ...
    #CJY at 2019.2.13 = 
    multiSelectShapes = pyqtSignal()

    #CJY at 2019.2.13  add SELECT 变2为3
    CREATE, EDIT,SELECT = list(range(3))
    ...
    def __init__(self, *args, **kwargs):
        ...
        #CJY at 2019.2.13 多选的shape保存在下面的列表中
        self.selectedShapes = []
    ...
    def mouseMoveEvent(self, ev):
        ...
        #CJY at 2019.2.13  多选框
        # Multiply select.
        if self.selecting():    #selecting
            self.overrideCursor(CURSOR_DEFAULT)    #CURSOR_DRAW
            if self.current:
                color = self.drawingLineColor
                if self.outOfPixmap(pos):
                    # Don't allow the user to draw outside the pixmap.
    ...
    def mouseReleaseEvent(self, ev):
        if ev.button() == Qt.RightButton:
            ...
            if self.drawing():
                self.handleDrawing(pos)
            #CJY at 2019.2.13 add MultiSelect
            if self.selecting():    #selecting
                self.handleDrawing(pos)
    ...
    def deSelectShape(self):     
        #CJY at 2019.2.14 取消多个
        for selectedShape in self.selectedShapes:
    ...
    #CJY at 2019.2.13 同时删除多个shape
    def MutideleteSelected(self):
        shapes = []
        for selectedShape in self.selectedShapes:
            if selectedShape:
                shape = selectedShape
    ...
    def finalise(self):
        ...
        self.current = None
        self.setHiding(False)
        #CJY at 2019.2.13 增加multiSelectShapes信号发射
        if self.drawing():
            self.newShape.emit()
        elif self.selecting():    #selecting
            self.multiSelectShapes.emit()
        self.update()
