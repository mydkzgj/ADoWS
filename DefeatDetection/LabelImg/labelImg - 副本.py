...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None, defaultSaveDir=None , transFilePath=""):
        ...
        dirpath = r"D:\myWork\clear\20181115412\20181115\1019\1\org"      
    ...
    def importInformation(self,argv=[]):
        ...
        information = []
        information.append("巡检编号:  "+ original_information[0])
        
        information.append("日期:         "+ original_information[1])
            
        information.append("大楼编号:  "+str(int(original_information[2])%1000))
            
        if original_information[3]=="1":
            information.append("大楼朝向:  "+"C")
        elif original_information[3]=="2":
            information.append("大楼朝向:  "+"B")
        elif original_information[3]=="3":
            information.append("大楼朝向:  "+"D")
        else:
            information.append("大楼朝向:  "+"A")
    
        subInformation = original_information[5].split("_")
        information.append("大楼位置:  "+"("+subInformation[2]+ "," +subInformation[3]+")")
        
        from base64 import b64decode
        base64_decrypt = b64decode(argv[2].encode('utf-8'))
        information.append("巡检人:      "+str(base64_decrypt,'utf-8'))
    
        information.append("拍照时间:  " + subInformation[4])
    
        detectionTime = subInformation[5].split(".")
        information.append("识别时间:  "+ detectionTime[0])
    ...
    def compareWithHistoricalImage(self, _value=False):       
        ...
        if(len(argv) == 4):            
            win_copy = labelImg_copy. MainWindow(argv[1] if len(argv) >= 2 else None,
...
# CJY 2018.12.06 增加参数argv_org,argv2,argv2_org
def get_main_app(argv=[],argv_org=[],argv2 = [],argv2_org = []):   
    """
    ...
    win.importLegend()
    if(len(argv_org)==3):
        win.importInformation(argv_org)        
    
    if(len(argv2)==4):
        win.importInformationForLabelCopy(argv2,argv2_org)
...
# CJY 2018.12.04 增加参数argv 以及 参数argv2
def main(argv = [] , argv_org = [] , argv2 = [] , argv2_org = [] ):
    '''construct main app and run it'''
    app, _win = get_main_app(argv,argv_org,argv2,argv2_org)
    return app.exec_()
