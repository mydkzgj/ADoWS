...
#0 表示该类不进入样本均衡的统计中
#1 表示该类进入样本均衡的统计中     设置为baseCropNum
#2.表示该类是负样本类              负样本需要与Category[0]进行均衡
#3.表示不生成该类                  设置为0
#4.表示                           置为后面跟着的数
#5.表示                           置为后面跟着的数*base
...
def ClassStatistics(xmlPath,configPath,baseCropNum):
    ...
    for i in range(len(Category)):
        if CategorySta[i] == 1:
            ...
            CropNum[Category[i]] = 0
        elif CategorySta[i] == 4:
            CropNum[Category[i]] = preset_CropN[i] if preset_CropN[i] > 0 else 0
        elif CategorySta[i] == 5:
            CropNum[Category[i]] = preset_CropN[i] * baseCropNum if preset_CropN[i] > 0 else 0
        else:
...
# CJY at 2020.8.21 修改原先需要计算mask的程序，改成固定size的crop
...
def CutFromHRImgByXml(xmlFPath, imageFPath, outputPath, cropBaseLength, cropNum, CutNormalFlag):
    ...
    for boxIndex in range(0, len(bbCoordinateList)):
        ...
        # crop图像左上角点的选择范围        分几种情况考虑
        if max(width, height) <= cropLength:
            xRange = [max(x2 - cropLength + 1, 0), min(x1, imgW - 1 - cropLength + 1)]
            yRange = [max(y2 - cropLength + 1, 0), min(y1, imgH - 1 - cropLength + 1)]
        elif width > cropLength and height <= cropLength:
            xRange = [max(x1 - cropHalfLength + 1, 0), min(x2-cropHalfLength, imgW - 1 - cropLength + 1)]
            yRange = [max(y2 - cropLength + 1, 0), min(y1, imgH - 1 - cropLength + 1)]
        elif width <= cropLength and height > cropLength:
            xRange = [max(x2 - cropLength + 1, 0), min(x1, imgW - 1 - cropLength + 1)]
            yRange = [max(y1 - cropHalfLength + 1, 0), min(y2-cropHalfLength, imgH - 1 - cropLength + 1)]
        else:
            xRange = [max(x1 - cropHalfLength + 1, 0), min(x2-cropHalfLength, imgW - 1 - cropLength + 1)]
            yRange = [max(y1 - cropHalfLength + 1, 0), min(y2-cropHalfLength, imgH - 1 - cropLength + 1)]

        if (xRange[0] >= xRange[1]) or (yRange[0] >= yRange[1]):
            print("Wrong Crop Range! Next Object")
            continue
        #保存路径
        ...
        while(iteration < max_iteration):
            ...
            # 随机生成左上角点坐标
            if success_crop_num == 0 and iteration == 1 and center[0] - cropHalfLength >= xRange[0] and center[0] - cropHalfLength <= xRange[1] and center[1] - cropHalfLength >= yRange[0] and center[1] - cropHalfLength <= yRange[1]:
            ...
            new_x1 = max(x1, cropBox[0])
            new_y1 = max(y1, cropBox[1])
            new_x2 = min(x2, cropBox[2])
            new_y2 = min(y2, cropBox[3])

            head_str = "" + new_head.format(objectname, new_x1 - cropBox[0], new_y1 - cropBox[1], new_x2 - cropBox[0], new_y2 - cropBox[1])
            # 确定是否有其他在crop框里的bBox
            wrong_crop = 0  # 错误剪切标志位
            for anotherboxIndex in range(0, len(bbCoordinateList)):
                if anotherboxIndex == boxIndex:
                    ...
                    if box_inter(cropBox, bbCoordinateList[anotherboxIndex]) == True:
                        # 交集占本身的比例
                        interR = cal_inter(bbCoordinateList[anotherboxIndex], cropBox) / max((a_x2 - a_x1) * (a_y2 - a_y1), 1E-12)
                        if (interR >= 0.2 and a_objectname == "crack") or (interR >= 0.5 and a_objectname != "crack"):
                            # 交集的坐标
                            new_a_x1 = max(a_x1, cropBox[0])
                            new_a_y1 = max(a_y1, cropBox[1])
                            new_a_x2 = min(a_x2, cropBox[2])
                            new_a_y2 = min(a_y2, cropBox[3])

                            head_str = head_str + new_head.format(a_objectname, new_a_x1 - cropBox[0], new_a_y1 - cropBox[1], new_a_x2 - cropBox[0], new_a_y2 - cropBox[1])
                            #print("Save another bbox with inter ratio = {} class: {}".format(interR, a_objectname))
                        else:
                            wrong_crop = 1
                            break

            if wrong_crop == 0:
                success_crop_num = success_crop_num + 1
                ...
                cropImgName = image_pre + '_{}_crop{}.JPG'.format(boxIndex, success_crop_num)
                cropSaveXml = prefix_str.format(objectname, cropImgName, savePath + '/' + cropImgName, cropLength,
                                                cropLength) + head_str + suffix
                cropedImg.save(savePath + '/' + cropImgName)
                open(savePath + '/' + image_pre + '_{}_crop{}.XMl'.format(boxIndex, success_crop_num), 'w').write(
                    cropSaveXml)

                cropedImg.save(SaveAllPath + cropImgName)
                open(SaveAllPath + image_pre + '_{}_crop{}.XMl'.format(boxIndex, success_crop_num), 'w').write(
                    cropSaveXml)

                cropedImg.save(SaveWithoutNormalPath + cropImgName)
                open(SaveWithoutNormalPath + image_pre + '_{}_crop{}.XMl'.format(boxIndex, success_crop_num),
                     'w').write(cropSaveXml)

                # cropedImg.save(SaveTrainPath + cropImgName)
                # open(SaveTrainPath + image_pre +'_{}_crop{}.XMl'.format(boxIndex,success_crop_num), 'w').write(cropSaveXml)


            if success_crop_num == cropNum[objectname]:
                print("iteration step: {}, croped_num: {}".format(iteration, success_crop_num))
                break

        if iteration >= max_iteration:
            print("Over max iteration, Early stop. Discard this object! Croped NUM {}".format(success_crop_num))
    ...
    if CutNormalFlag == 1:
        ...
        for boxIndex in range(0, len(normal_bbCoordinateList)): 
            ...
            while (iteration < max_iteration):
                ...
                # 随机生成左上角点坐标
                if success_crop_num == 0 and iteration == 1 and center[0] - cropHalfLength >= xRange[0] and center[
                    0] - cropHalfLength <= xRange[1] and center[1] - cropHalfLength >= yRange[0] and center[
                    1] - cropHalfLength <= yRange[1]:
                    cropLTpoint = [center[0] - cropHalfLength, center[1] - cropHalfLength]
                else:
                    cropLTpoint = [random.randint(xRange[0], xRange[1]), random.randint(yRange[0], yRange[1])]
                ...
                for anotherboxIndex in range(0, len(bbCoordinateList)):
                    if anotherboxIndex == boxIndex:
                        ...
                        if box_inter(cropBox, bbCoordinateList[anotherboxIndex]) == True:
                            # 交集占本身的比例
                            interR = cal_inter(bbCoordinateList[anotherboxIndex], cropBox) / max(
                                (a_x2 - a_x1) * (a_y2 - a_y1), 1E-12)
                            if interR > 0 and a_objectname != "normal":
                                wrong_crop = 1
                                break
                ...
                if wrong_crop == 0:
                    ...
                    cropImgName = image_pre + '_N{}_crop{}.JPG'.format(boxIndex, success_crop_num)
                    cropSaveXml = prefix_str.format(objectname, cropImgName, savePath + '/' + cropImgName, cropLength,
                                                    cropLength) + head_str + suffix
                    cropedImg.save(savePath + '/' + cropImgName)
                    open(savePath + '/' + image_pre + '_N{}_crop{}.XMl'.format(boxIndex, success_crop_num), 'w').write(
                        cropSaveXml)

                    cropedImg.save(SaveAllPath + cropImgName)
                    open(SaveAllPath + image_pre + '_N{}_crop{}.XMl'.format(boxIndex, success_crop_num), 'w').write(
                        cropSaveXml)

                    #cropedImg.save(SaveWithoutNormalPath + cropImgName)
                    #open(SaveWithoutNormalPath + image_pre + '_N{}_crop{}.XMl'.format(boxIndex, success_crop_num), 'w').write(cropSaveXml)

                    # cropedImg.save(SaveTrainPath + cropImgName)
                    # open(SaveTrainPath + image_pre +'_{}_crop{}.XMl'.format(boxIndex,success_crop_num), 'w').write(cropSaveXml)

                if success_crop_num == normalCropNum:
                    print("iteration step: {}, croped_num: {}".format(iteration, success_crop_num))
                    break
